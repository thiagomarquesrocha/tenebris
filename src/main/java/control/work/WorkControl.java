...
public class WorkControl {
	...
	public static void findById(WorkDao dao, Object[] o) {
		...
		try {
			...
			String sql = String.format("SELECT * FROM (SELECT a.*, b.avaliacao, c.nome as autor FROM (SELECT a.id, a.autor as autorId, a.resumo, a.data, a.titulo as titulo, a.imagem as path, a.tipo, b.nome as area, c.nome as instituicao, c.id as instituicaoId, d.nome as obraTipo FROM obra a, area b, instituicao c, tipo_obra d WHERE a.area=b.id AND c.id = a.instituicao AND a.id = %s AND a.tipo = d.id) a LEFT OUTER JOIN (SELECT * FROM avaliacao WHERE obra = %s AND usuario = %s) b ON a.id = b.obra, autor c WHERE c.id = a.autorId) a LEFT OUTER JOIN (SELECT obra, avg(avaliacao) as media FROM avaliacao GROUP BY obra) b ON a.id = b.obra GROUP BY a.id", work, work, user);
